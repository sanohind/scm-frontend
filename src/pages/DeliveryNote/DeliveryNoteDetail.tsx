...
import { API_indexDNSupplier, API_indexDNDetailUpdateSupplier } from '../../api/api';
...
const DeliveryNoteDetail = () => {
  ...
  const fetchDeliveryNotes = async () => {
    ...
    try {
      ...
      if (result && result.data) {
        const dn = result.data.find(dn => dn.no_dn === noDN);
        if (dn) {
          ...
          Swal.fire('Error', 'Delivery Note not found.', 'error');
        }
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_indexDNDetailUpdateSupplier}`, {
        method: 'PUT',
        headers: {
